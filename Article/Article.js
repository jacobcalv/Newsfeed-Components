...
const articles = document.querySelector('body .articles')
function articleMaker(item) {
  ...
  article.textContent = data['firstParagraph'];
 

  const p2 = document.createElement('p');
  article.textContent = data['secondParagraph'];
  

  const p3 = document.createElement('p');
  article.textContent = data['thirdParagraph']
...
let newArray = data.map(function(item){
  let newArticle = articleMaker(item.title);
  return newArticle;
})

data.forEach((item) => {
  let nextArticle = articleMaker(item);
